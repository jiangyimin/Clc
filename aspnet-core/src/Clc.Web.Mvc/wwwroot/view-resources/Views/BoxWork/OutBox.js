...
function addBox() {
    ...
    abp.services.app.boxRecord.outBox(taskRow.id, $('#boxId').val(), work.me.workers).done(function(ret) {
        if (ret != null) {
            abp.notify.error(ret);
            return;
        };

        abp.notify.success("成功出箱");
...
finput.boxScanned = function(rfid) {
    ...
    //alert(taskRow.id+rfid+work.me.workers);
    abp.services.app.boxRecord.outBox(taskRow.id, rfid, work.me.workers).done(function(ret) {
        if (ret != null) {
            abp.notify.error(ret);
            return;
        };
